...
def analyze_intent_and_discover_explanation_codes(
    ...
    """
    logger.info("[TOOL: INTENT] Extracting scenario parameters and running vector search...")
    llm = ChatOpenAI(
    ...
    system_instruction = (
        "You are an expert AML Compliance Analyst. Extract structured scenario parameters from the user prompt.\n"
        "Output ONLY a valid JSON object matching AMLIntent schema with mandatory root fields:\n"
        "- scenario_name (str): descriptive title\n"
        "- scenario_type (str): 'CUSTOMER', 'TRANSACTION', or 'ACCOUNT'\n"
        "- transaction_type (str or null): explicit transaction type e.g. 'CASH DEPOSIT'\n"
        "- detection_logic (str): summary of business logic\n"
        "- thresholds (list of dicts)\n"
        "- time_window (dict with unit and value)\n"
    ...
    try:
        ...
        elif "aml_intent" in intent_dict and isinstance(intent_dict["aml_intent"], dict):
        ...
        if not intent_dict.get("detection_logic"):
            intent_dict["detection_logic"] = intent_dict.get("scenario_name", user_prompt)
        ...
        if tx_type:
            ...
            if discovered_codes:
                expl_checkpoint = format_explanation_code_checkpoint(tx_type, discovered_codes)
                code_strings = [str(c.get("code")).strip() for c in discovered_codes if c.get("code")]
...
def generate_scenario_execution_plan(
    ...
    AFTER calling this tool, present the full plan (including the explanation codes table if provided)
    to the user and STOP to explicitly ask for their approval.
    Do NOT call execute_oracle_dwh_shadow_test until the user explicitly accepts or approves this plan.

    Args:
        intent_json (str): Serialized AMLIntent payload dictionary JSON string.
        explanation_code_checkpoint (Optional[str]): Markdown table of discovered explanation codes
            from the prior analyze_intent_and_discover_explanation_codes call. Included in the plan
            if provided, so the user sees codes alongside plan details.
    ...
    expl_section = (
        f"\n\n## Discovered Explanation Codes\n{explanation_code_checkpoint}"
        if explanation_code_checkpoint
        else ""
    )

    prompt = (
        "Generate a clear, professional markdown implementation plan for this AML scenario.\n"
        "Include the following sections:\n"
        "1. Scenario Overview\n"
        "2. Detection Logic Summary\n"
        "3. Observation Window & Time Parameters\n"
        "4. Thresholds & Alert Conditions\n"
        "5. Explanation Codes (if provided)\n"
        "6. Shadow Testing Scope\n\n"
        f"Intent Payload:\n{intent_json}"
        f"{expl_section}"
    )
...
def execute_oracle_dwh_shadow_test(intent_json: str) -> str:
    ...
    logger.info("[TOOL: SQL_BRIDGE] SQL extracted (%d chars). Executing shadow test...", len(sql))
    ...
    try:
        ...
        ratio = round(detail_count / max(header_count, 1), 2) if header_count > 0 else 0.0
...
def persist_and_validate_scenario_in_dwh(intent_json: str, raw_sql: str) -> str:
    ...
    """
    logger.info("[TOOL: PERSISTER] Writing confirmed scenario to PIO_AML_PRODUCTION_SCENARIOS...")
    ...
    time_window_days: Optional[int] = tw_value * unit_to_days.get(tw_unit, 1) if tw_value else None
